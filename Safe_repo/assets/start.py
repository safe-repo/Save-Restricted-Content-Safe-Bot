...
async def start(event):
    ...
    collection.update_one({"user_id": user_id}, {"$set": {"user_id": user_id}}, upsert=True)
    buttons = [
        [Button.url("Join Channel", url="https://t.me/safe_repo")],
        [Button.url("Contact Me", url="https://t.me/Safe_repo")],
    ]
...
async def plan_command(event):
    ...
    buttons = [
        [Button.url("Send Gift Card Code", url="https://t.me/safe_repo_bot")]
    ]
...
async def term_command(event):
    # Creating inline keyboard with buttons
    buttons = [
        [Button.url("Query?", url="https://t.me/Safe_repo_bot"),
         Button.url("Channel", url="https://telegram.dog/safe_repo")]
    ]
...
async def youtube_dl_command(_, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            caption = f"{video_info['title']}\n\n__**Powered by [Save restricted Content safe Bot](https://t.me/Save_restricted_Content_safe_Bot)**__"  # Set caption to the title of the video
