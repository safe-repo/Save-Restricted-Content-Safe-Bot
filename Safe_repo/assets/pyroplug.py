...
async def send_video_with_chat_id(client, sender, path, caption, duration, hi, wi, thumb_path, upm, is_pinned):
    ...
    try:
        result = await client.send_video(
            ...
            progress_args=(
                client,
                '**__Uploading: [Safe_repo](https://t.me/Safe_repo)__**\n ',
                upm,
                time.time()
            )
        )
...
async def send_document_with_chat_id(client, sender, path, caption, thumb_path, upm, is_pinned):
    ...
    try:
        result = await client.send_document(
            ...
            progress_args=(
                client,
                '**__Uploading:__**\n**Bot made by https://t.me/Safe_repo**',
                upm,
                time.time()
            )
        )
...
async def get_msg(userbot, client, sender, edit_id, msg_link, i, file_n):
    ...
    if 't.me/c/'  in msg_link or 't.me/b/' in msg_link:
        ...
        try:
            ...
            file = await userbot.download_media(msg, progress=progress_for_pyrogram, progress_args=(client, "**__Unrestricting__: __[Safe Repo](https://t.me/Safe_repo)__**\n ", edit, time.time()))            # Retrieve user's custom renaming preference if set, default to '@Safe_repo' otherwise
            if not file:
